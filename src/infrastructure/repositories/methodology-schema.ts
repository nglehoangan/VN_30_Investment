...
const optionalText = z.string().refine(value => value.trim() === value && !/[\x00-\x1f]/.test(value));
...
const schema = z.strictObject({
  ...
  approvalReference: optionalText, governanceStatus: z.enum(["DRAFT", "PROPOSED", "APPROVED", "RETIRED"]),
  intendedUse: z.enum(["PRODUCTION", "TEST"]), effectiveDate: validDate, configurationReference: text,
...
const expected = Object.freeze({
  ...
  semanticVersion: "Semantic version MAJOR.MINOR.PATCH with optional prerelease/build identifiers", approvalReference: "External approval evidence reference; empty until approved",
  governanceStatus: "DRAFT, PROPOSED, APPROVED or RETIRED", intendedUse: "PRODUCTION or TEST",
...
export function validateMethodology(value: unknown): Readonly<MethodologyRecord> {
  ...
  if (row.governanceStatus === "APPROVED" && !row.approvalReference) {
    throw new ValidationError([{ field: "approvalReference", reason: "APPROVED_REQUIRES_EXTERNAL_EVIDENCE", expected: "Nonempty external approval reference" }]);
  }
  if (row.governanceStatus !== "APPROVED" && row.approvalReference) {
